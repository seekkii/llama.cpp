...
struct InferenceState {
    ...
    decltype(ggml_threadpool_free)* ggml_threadpool_free_fn = nullptr;
};
int my_main(ThreadSafeQueue<std::string> *input_queue,ThreadSafeQueue<std::string> *output_queue, int argc, char ** argv) {
    common_params params;
    g_params = &params;
    ...
    auto state = std::make_shared<InferenceState>();
    state->params = std::move(params);
    state->ctx = llama_init.context.release();
    state->model = llama_init.model.release();
    // state->smpl = llama_init.
    state->input_queue = input_queue;
    state->output_queue = output_queue;
    state->threadpool = threadpool;
    state->threadpool_batch = threadpool_batch;
    state->ggml_threadpool_free_fn = ggml_threadpool_free_fn;
    state->chat_templates = std::move(chat_templates);  // move the shared_ptr in

std::thread([state]() {
    ThreadSafeQueue<std::string>* input_queue = state->input_queue;
    ThreadSafeQueue<std::string>* output_queue = state->output_queue;
    common_sampler * smpl = nullptr;
    g_smpl = &smpl;
    ...
    // auto& embd_inp = state->embd_inp;
    auto& chat_templates = state->chat_templates;  // safe to use
    ...
    while ((n_remain != 0 && !is_antiprompt) || params.interactive) {
        ...
        if (input_echo && display) {
            for (auto id : embd) {
                ...
                // if (token_str.empty()) {
                //     std::cerr << "[WARN] empty or invalid token id=" << id << std::endl;
                //     continue;
                // }
                // if (!is_valid_utf8(token_str)) {
                //     // Log the bad bytes in hex so you can inspect them
                //     std::cerr << "[UTF8 ERR] token id=" << id << " bytes=";
                //     for (unsigned char c : token_str) {
                //         std::cerr << std::hex << (int)c << ' ';
                //     }
                //     std::cerr << std::dec << " as string=\"" << token_str << "\"" << std::endl;
                // }
                // else{
                    output_queue->push(token_str);
                // }
        ...
        if ((int) embd_inp.size() <= n_consumed) {
            ...
            if ((n_past > 0 || waiting_for_first_input) && is_interacting) {
                ...
                bool done = false;
                while (!done) {
                    auto maybe = input_queue->pop_for(std::chrono::milliseconds(100));
                    if (!maybe) {
                        // no input yet—do any periodic background work here, then retry
                        continue;
                    }

                    // 2) process the input
                    const std::string currentInput = std::move(*maybe);
                    buffer += currentInput;
                    done = true;
                    // 3) shutdown check
                ...
                std::cerr << "[loop] about to pop_for\n" << std::flush;
                // do {
    ...
    llama_backend_free();
    auto * ggml_threadpool_free_fn = state->ggml_threadpool_free_fn;
    ggml_threadpool* threadpool = state->threadpool;
    auto* threadpool_batch = state->threadpool_batch;
...
int wrapped_main(ThreadSafeQueue<std::string> *input_queue,ThreadSafeQueue<std::string> *output_queue, std::vector<std::string>& args) {
    std::cout << "Calling main with args:\n";
    ...
    return my_main(input_queue, output_queue,static_cast<int>(args.size()), c_args.data());
}
